...
# read in sustained smoking associated cpgs
sust <- read.delim(gzfile(paste0("/home/groups/hoolock2/u0/bd/Projects/",
    "lyndsey_shorey_project/pmid_34429407_data/sustained_MSDP_cpgs.txt"))
)
# subset beta vals matrix to contain sustained smoking cpgs
beta.sust <- beta[rownames(beta) %in% sust$CpG, ]
# returns 803 cpgs
...
mydata <- prepare_mat(beta.sust, pd)

#-------------------------------------------------------------------------------------------------------------------#

# Store hyperparameter values
penalty_val <- 1.27427498570313E-08
nCpG_val <- 98
...
saveRDS(lasso_fit, "model3_lasso_fit.RDS")
# can be re-loaded with: my_fitted_model <- readRDS("model3_lasso_fit.RDS")
...
important_cpg_barplot(var_imp_file = paste0(hyper_string, "_nonzero_predictors.txt"),
    ...
    plot_title = "LASSO Model 3 PACE Importance CpGs",
...
write.table(my_mets,
  "model3_vcsip_performance_metrics.txt",
  sep="\t",
  col.names=TRUE,
  row.names=FALSE,
  quote=FALSE)
...
p <- roc_curve(pred_res, status, prob_nonsmoker) %>%
  ...
  coord_equal() +
  theme_bw() +
  ggtitle("Model 3 EPIC ROC")
ggsave(filename="model3_vcsip_roc.pdf", p)
