...
library(xgboost)    # to get variable importance from gradient boost
...
option_list = list(
  ...
  make_option(c("-c", "--hyperparameter3"), type="numeric", default=NULL,
              help="value for hyperparameter3", metavar="character"),
...
ntree_val <- opt$hyperparameter1
depth_val <- opt$hyperparameter2
nCpG_val <- opt$hyperparameter3
# Store hyperparameter value string for file names
hyper_string <- paste0("nCpG_", nCpG_val, "_ntree_", ntree_val, "_depth_", depth_val)
...
mod_spec <- boost_tree(mode="classification", trees=ntree_val, min_n=min_n, tree_depth=depth_val) %>%
    set_engine("xgboost")
